...
/******declare funcss********/
task_t* custom_pick_next(task_t* prev);
//task_t* locate_next_member(task_t* prev,task_t* leader,runqueue_t *rq);
//int get_active_member_prio(task_t* prev,task_t* leader,runqueue_t *rq);
//int get_expired_member_prio(task_t* leader,runqueue_t* rq);
int check_has_character(task_t* proc);
/*****************donee*****************/
...
/* my costum scheduling functions----------- and the start of my changes*************/
task_t* custom_pick_next(task_t* curr){
	task_t* next_task = curr;
	int max_prio = MAX_PRIO + 1;
	//proccess is a party member, locate next proccess
    task_t* leader =  curr->group_leader;
	//int prio = curr->prio;
	struct player *entry;
	...
	list_for_each_safe(position,tmp, &(leader->party_list)){
        ...
        task_t* task = find_task_by_pid(entry->player_pid);
		//printk(KERN_INFO "#############CHECKING MY PARTY MEMBER WITH PID %d AND PRIO %d\n",task->pid,task->prio);
		if(task->state == TASK_RUNNING && task->pid != curr->pid){
			int task_prio = task->prio;
			if(task_prio < max_prio){
				max_prio = task_prio;
				next_task = task;	
			}
		}
	}
	//printk(KERN_INFO "#############IM CHOSE MEMBER WITH PID %d\n",next_task->pid);
	return next_task;	
}
...
/*task_t* locate_next_member(task_t* prev, task_t* leader, runqueue_t *rq) {
    prio_array_t *array;
    int active_prio = get_active_member_prio(prev, leader, rq);
	printk(KERN_INFO "#############ACTIVE PRIO IS   %d\n",active_prio);

    if (active_prio == NO_MEMEBER_FOUND) {
        int expired_prio = get_expired_member_prio(leader, rq);
		printk(KERN_INFO "#############EXPIRED PRIO IS   %d\n",active_prio);
        if (expired_prio == NO_MEMEBER_FOUND) {
            return NULL;
        } else {
            array = rq->expired;
            struct list_head *list = array->queue + expired_prio;
            struct list_head *pos, *tmp;
            list_for_each_safe(pos, tmp, list) {
                task_t *task = list_entry(pos, task_t, run_list);
                if (task->group_leader == leader) {
					//MAYBE WE NEED TO ADD THE TASK TO ACTIVE QUEUE?
					dequeue_task(task,rq->expired);
        ...
        array = rq->active;
        struct list_head *list = array->queue + active_prio;
...
int get_active_member_prio(task_t* prev,task_t* leader,runqueue_t *rq){
    //check if any members running 
    struct player *entry;
	struct list_head* tmp;
	struct list_head* position;
    task_t* task;
    int prio;
    int min_prio = current->prio;
	//find a lowest prio active member
    list_for_each_safe(position,tmp, &(leader->party_list)){
		entry = list_entry (position, struct player, my_list);
        task = find_task_by_pid(entry->player_pid);
        if(task->array == current->array){
			printk(KERN_INFO "#############IM CHECKING IN ACTIVE MEMBER WITH PID %d AND STATE %d\n",task->pid,task->state);
            prio = task->prio;
            if(prio < min_prio){
                min_prio = prio;
            }
    ...
    if(min_prio <  current->prio){
        return min_prio;
    }else{
        return NO_MEMEBER_FOUND;
    }
}

int get_expired_member_prio(task_t* leader,runqueue_t* rq){
    //check if any members in the expired queue
    struct player *entry;
	struct list_head* tmp;
	struct list_head* position;
    task_t* task;
    ...
    list_for_each_safe(position,tmp, &(leader->party_list)){
		entry = list_entry (position, struct player, my_list);
        task = find_task_by_pid(entry->player_pid);
		printk(KERN_INFO "#############IM CHECKING MEMBER WITH PID %d AND STATE %d\n",task->pid,task->state);
        if(task->array =! NULL){
            prio = task->prio;
			printk(KERN_INFO "#############IM CHECKING MEMBER IN EXP WITH PID %d AND STATE %d\n",task->pid,task->state);
            if(prio < min_prio){
...
pick_next_task:
	...
	/*I added my own function to pick the next_task */
	idx = sched_find_first_bit(array->bitmap);
	queue = array->queue + idx;	
	next = list_entry(queue->next, task_t, run_list); // this will be my default setting
	

	if(check_has_character(current)){
		//get the lowest running task
		task_t * next_task = custom_pick_next(current);
		if(next_task->state == TASK_RUNNING){
			if(next_task->array != current->array){
				//task is in expired, add to active array
				dequeue_task(next_task, next_task->array);
				enqueue_task(next_task, rq->active);
			}
			//printk(KERN_INFO "#############MY NEXT IS WITH PID %d INSTEAD OF %d\n",next_task->pid, next->pid);
			next = next_task;
		}
